...
export default function Rewards() {
  ...
  useEffect(() => {
    api.get<Reward[]>('/rewards').then(setRewards).catch(console.error).finally(() => setLoading(false))
  }, [])
  ...
  const saveEdit = async () => {
    ...
    try {
      const updated = await api.patch<Reward>(`/rewards/${editing}`, { name: newName })
      setRewards((prev) => prev.map((p) => (p.id === updated.id ? updated : p)))
      setEditing(null)
    } catch (e) {
      console.error(e)
    }
  ...
  const create = async (e: React.FormEvent) => {
    ...
    try {
      const created = await api.post<Reward>('/rewards', { name: createName.trim() })
  ...
  const remove = async (id: number) => {
    try {
      ...
      setRewards((prev) => prev.filter((p) => p.id !== id))
    } catch (e) {
      console.error(e)
    } finally {
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold text-ink mb-8">Rewards</h1>
